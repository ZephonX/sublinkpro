...
- **Image:** `ghcr.io/zephonx/sublinkpro` (stable) or `ghcr.io/zephonx/sublinkpro:dev` (dev/preview builds).
...
Dev/preview build: replace the image with `ghcr.io/zephonx/sublinkpro:dev`.
...
# docker run install:
docker stop sublinkpro && docker rm sublinkpro
docker rmi ghcr.io/zephonx/sublinkpro            # optional: remove image
